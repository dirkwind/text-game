...
def add_item(item='item name or [purpose, name, value, turns/dodgable/params, uses]', purpose='purpose', item_value=0, turns=1, uses=1, silent=False):
    '''Add an item to the player's inventory. purpose='heal', 'bonus attack', 'bonus defense', 'damage', 'special' '''
    added = False
    if type(item) != list:
        item_name = item
        item = [item, purpose, item_value, turns, uses]
    else:
        item_name = item[1]
    ...
    if len(player['inventory']) < 8:
        if not added: player['inventory'].append(item)
        if not silent:
            text_scroll(f'\n{item_name.upper()} has been added to your inventory!\n')
    else:
        if not silent:
...
def battle(enemy):
    ...
    while in_battle:
            ...
            if len(inventory) == 0:
                ...
                while choosing:
                    ...
                    for item in inventory:
                        print(f'{i + 1} : {item[1].upper()} x{item[4]}')
                        i += 1
                ...
                if cont_item:
                    ...
                    if inventory[item_index][4] <= 1:
                        item = inventory.pop(item_index)
                    else:
                        item = inventory[item_index]
                        inventory[item_index][4] -= 1
                    purpose = str(item[0])
                    name = str(item[1])
                    if type(item[2]) == type(add_item):
                        value = item[2]
                    else:
                        value = int(item[2])
                    if type(item[3]) == int:
                        num_of_turns = item[3]
                    elif type(item[3]) == list:
                        params = item[3]
                    else:
                        dodgable = item[3]
                    turns += 1

                    if purpose == 'bonus attack' or purpose == 'bonus defense':
                        items_in_use.append((purpose, value, turns + num_of_turns, name))           
                    ...
                    if purpose == 'heal':
                        ...
                        if player['health'] > player['max_health']:
                            ...
                            text_scroll(f'\nYou gained {value} HP!\n')
                    elif purpose == 'bonus attack':
                        bonus_damage += int(value)
                        text_scroll(f'\nATTACK increased by {value} for {num_of_turns} turn(s)!\n')
                    elif purpose == 'bonus defense':
                        bonus_def += value
                        text_scroll(f'\nDEFENSE increased by {value} for {num_of_turns} turn(s)!\n')
                    elif purpose == 'damage':
                        chance_num = random.randint(1, 100)
                        if chance_num >= int((enemy['speed'] / 20) * 100) or not dodgable:
                            item_damage = value - enemy['defense']
                            if item_damage < 0: item_damage = 0
                            enemy['health'] -= item_damage
                            text_scroll(f'\nYou did {item_damage} damage to {enemy_name}! {enemy_name} is now at {enemy["health"]} HP!\n')
                        else:
                            text_scroll(f'\n{enemy["name"]} dodged the {name.upper()}!\n')
                    elif purpose == 'special':
                        
                        if '&turns' in params:
                            turns = value(configure_params(params, enemy, turns))
                        else:
                            value(configure_params(params, enemy, turns))
        ...
        for itm in items_in_use:
            if turns >= itm[2]:
                items_in_use.remove(itm)
                text_scroll(f'Your item, {itm[3].upper()}, has worn off!\n')
                if itm[0] == 'bonus attack':
                    bonus_damage -= itm[1]
                else:
                    bonus_def -= itm[1]
...
player = {
    ...
    'fled': 0,
    'inventory': [
        #[purpose, name, value, turns/dodgable/params, uses]
        #purposes are 'heal', 'bonus attack', 'bonus defense', 'damage', 'special'
        ['heal', 'Bandage', 10, None, 2],
        ['bonus attack', 'Energy Drink', 10, 3, 3],
        ['bonus defense', 'Frail Shield', 8, 2, 1, 1],
    ]
}
